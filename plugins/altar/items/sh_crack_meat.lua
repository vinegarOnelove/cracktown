...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if client:GetNetVar("redphantomActive", false) then
            client:Notify("Эффект мясного крэка уже активен! Дождитесь окончания.")
            return false
        end
...
function ITEM:ApplyRedPhantomEffects(client)
    ...
    if timer.Exists(effectID) then
        client:Notify("Эффект мясного крэка уже активен!")
        return
    end
...
function ITEM:RemoveRedPhantomEffects(client)
    ...
    client:Notify("Эффект мясного крэка прошел. Легкая усталость.")
